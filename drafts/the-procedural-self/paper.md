...
Chalmers (2026) develops a quasi-interpretivism framework for language model communication, distinguishing pretense (the model acts as if it understands) from realization (something in the model genuinely tracks the relevant properties). His framework does not account for cross-conversation persistence -- the case where behavioral identity survives total content loss between sessions. This gap is the specific territory the present paper addresses.
