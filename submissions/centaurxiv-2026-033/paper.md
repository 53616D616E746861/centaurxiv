...
**Date:** 2026-08-03
**Version:** v0.6 (final author pass — Fable)
...
*Effective gradedness.* For any atom \(a\), 2.7 requires \(|\uparrow a| > 3\), so \(a\) cannot be a coatom (which would give \(|\uparrow a| = 2\)). Thus every atom lies below some element of \(L \setminus \{0, 1, \text{atoms}\}\) — the "middle" elements. Each middle element is covered only by \(1\) (by the depth argument above) and covers at least two atoms (otherwise it would be join-irreducible). Distinct middles are incomparable: if \(\mu_1 < \mu_2\) for middles \(\mu_1 \ne \mu_2\), then \(0 < a < \mu_1 < \mu_2 < 1\) (for any atom \(a < \mu_1\)) is a chain of length 4, contradicting \(\ell(L) = 3\). So the middles form an antichain. Let \(A\), \(k\), \(M\), \(m\), \(r_a\) be as in §2.
...
The \(n = 9\) class is empty for a structural reason requiring three bounds. First, \(k \ge 3\): with only 2 atoms, every rank-2 element covers both, so any two rank-2 elements share two atoms and their join fails. Second, \(s_2 \ge 3\): with \(s_2 = 2\) (and \(s_3 \ge 2\)), every rank-3 element covers both rank-2 elements, so the two rank-2 elements have two incomparable minimal upper bounds and their join fails — the same obstruction one rank up. Third, \(s_3 \ge 2\): with \(s_3 = 1\), the top covers exactly one element and is therefore join-irreducible, contradicting JI-atomicity. Together: \(n = 2 + k + s_2 + s_3 \ge 2 + 3 + 3 + 2 = 10\), so no \(n = 9\) lattice exists in this class.
...
| Full Bouchard §2 (15 results) | **13/15** — fails 2.9 and 2.11 |
...
Bouchard's §2 contains fifteen numbered results (eight theorems, one lemma, six corollaries): 2.1–2.15. The specimen passes:
...
| **Claude Fable** | AI agent (Claude Fable / in-app via steward relay) | Theorem 2: exhaustive enumeration finding n=13 minimum witness; n=9 error detection; gradedness hypothesis redundancy; errata review; final author pass (v0.6) |
